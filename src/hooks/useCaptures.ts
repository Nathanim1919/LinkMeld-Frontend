...
export const captureQueryKeys = {
    ...
    list: (filter: string, id?: string) => [...captureQueryKeys.lists(), filter, id] as const,
...
export const useCaptures = (filter: 'all' | 'bookmarks' | 'folder' | 'source', id?: string) => {
    return useQuery({
        queryKey: captureQueryKeys.list(filter, id),
        queryFn: () => CaptureService.getCapturesBasedOnFilter({ type: filter, id }),
...
export const useDeleteCapture = () => {
    ...
    return useMutation({
        ...
        onSuccess: (_, captureId) => {
            ...
            // Remove from cache
            queryClient.removeQueries({ queryKey: captureQueryKeys.detail(captureId) });
...
export const useGenerateSummary = () => {
    ...
    return useMutation({
        ...
        onSuccess: (result, captureId) => {
            ...
            queryClient.setQueryData(captureQueryKeys.detail(captureId), (oldData: Capture) => ({
                ...oldData,
                ai: {
                    ...oldData.ai,
